...
/**  Definições da funções da biblioteca **/
    ...
    **  de alunos */
    ...
    void lista_manipula (Lista *lista, Registros dados, Registros novo, char opcao){
        Aluno *abuscar = NULL;
        ...
        for (abuscar = lista->primeiro; abuscar != NULL; abuscar = abuscar->proximo){
            if (abuscar->Dados.Matricula == dados.Matricula){
                if (opcao == 'R'){
                    ...
                    abuscar->Dados.Matricula = novo.Matricula;
                    string_zera (abuscar->Dados.Nome, 0, TAM_MAX);
                    strcpy (abuscar->Dados.Nome, novo.Nome);
                    abuscar->Dados.OP = novo.OP;
                    abuscar->Dados.Turma = novo.Turma;
                    strcpy (abuscar->Dados.Curso, novo.Curso);
                }
            }
        }
    }
    ...
    void lista_merge (FILE *arq, IndicePrimario *l1, IndicePrimario *l2){
        Elempri *aux1 = l1->inicio, *aux2 = l2->inicio;

        while ((aux1 != NULL) && (aux2 != NULL)){
            if (aux1 == NULL){
                reg_escreve (arq, *(aux2->infos));
                aux2 = aux2->proximo;

            }else if (aux2 == NULL){
                reg_escreve (arq, *(aux1->infos));
                aux1 = aux1->proximo;

            }else if (strcmp(aux1->chave, aux2->chave) < 0){
                reg_escreve (arq, *(aux1->infos));
                aux1 = aux1->proximo;
